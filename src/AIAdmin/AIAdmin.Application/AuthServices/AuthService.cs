...
using AIAdmin.Application.AuthServices.Dtos;
using Flurl;
using Flurl.Http;
...
using AIAdmin.Core.Cache;
using AIAdmin.Core.DataEncryption.Encryptions;
using AIAdmin.Core.DataEncryption.Extensions;
using AIAdmin.Multiplex.AdminUser;
using AIAdmin.Multiplex.AI;
using AIAdmin.Multiplex.Contracts.IAdminUser;
using AIAdmin.Multiplex.Contracts.IAdminUser.Models;
using AIAdmin.Multiplex.Contracts.IAdminUser.OAuth2;

namespace AIAdmin.Application.AuthServices;
/// <summary>
/// 用户授权服务
/// </summary>
...
public class AuthService(IAdminCache cache, Kernel kernel, IOAuth2UserManager oAuth2UserManager, IHubContext<AuthorizationHub, IAuthorizationClient> hubContext, IConfiguration configuration, IAdminToken adminToken, IHttpContextAccessor httpContextAccessor, ISqlSugarClient db, ICurrentUser currentUser) : ApplicationService
{
    ...
    {
        if (_kernel.Services.GetService(typeof(IChatCompletionService)) is null)
        {
            throw PersistdValidateException.Message("AI 功能未配置，请在 appsettings 中填写 AIOptions（Endpoint、ApiKey、ModelName）");
        }
